...
export async function POST(
  ...
  try {
    ...
    if (!endpoint || !endpoint.channel) {
      pushLogger.warn(traceId, 'PushRequest', 'Endpoint not found', { endpointId: id })
      return new Response(
        JSON.stringify(
          createPushResponse('failed', '接口不存在', traceId)
        ),
    ...
    if (endpoint.status !== 'active') {
      pushLogger.warn(traceId, 'PushRequest', 'Endpoint is disabled', { endpointId: id })
      return new Response(
        JSON.stringify(
          createPushResponse('failed', '接口已禁用', traceId)
        ),
    ...
    // 异步模式：立即返回 202，后台处理
    if (isAsync) {
      pushLogger.info(traceId, 'PushRequest', 'Async mode: returning 202 immediately')
      ...
      ;(async () => {
        try {
          ...
          pushLogger.info(traceId, 'PushRequest', 'Scheduling callback dispatch', {
            callbackUrl,
            callbackTimeout,
            status
          })
          ...
          pushLogger.error(traceId, 'PushRequest', 'Background task error', error)
        }
      })()
    ...
    const message = result.success ? '推送成功' : result.error || '推送失败'
    const status = result.success ? 200 : result.error === '推送超时' ? 504 : 500
    const responseStatus = result.success ? 'success' : 'failed'

    if (result.success) {
      pushLogger.info(traceId, 'PushRequest', 'Sync push completed successfully')
    } else if (status === 504) {
    ...
    return new Response(
      JSON.stringify(
        createPushResponse('failed', error instanceof Error ? error.message : '推送失败', traceId)
      ),
