...
export class BarkChannel extends BaseChannel {
    ...
    templates: [
      ...
      },
      {
        ...
        fields: [
          ...
          {
            key: 'device_keys',
            description: 'key数组，用于批量推送',
            required: false,
            component: 'input',
            placeholder: '多个key以逗号分隔，将会自动转换为数组'
          },
          {
            key: 'group',
            description: '消息分组',
            required: false,
            component: 'input',
            placeholder: '对消息进行分组，在通知中心中显示'
          },
          {
            key: 'icon',
            description: '自定义图标URL',
            required: false,
            component: 'input',
            placeholder: '自定义图标URL，图标将自动缓存到本机'
          },
          {
            key: 'sound',
          ...
          {
            key: 'url',
            description: '点击推送时跳转的URL',
            required: false,
            component: 'input'
          },
          {
            key: 'copy',
            description: '复制推送时，指定复制的内容',
            required: false,
            component: 'input',
            placeholder: '不传则复制整个推送内容'
          },
          {
            key: 'level',
            description: '推送中断级别',
  ...
  async sendMessage(
    ...
    if (message.device_keys) {
      postData.device_keys = message.device_keys
        .split(',')
    ...
    if (message.action && message.action !== 'default')
      postData.action = message.action;
